...
\title{Design and Implementation of a Smart Academic Companion for Engineering Students Using AI, LayoutLMv3, MiniLMv6, spaCy, and Intelligent Scheduling (TrackEneer)
}
...
\begin{enumerate}
    \item \textbf{Automate Syllabus and Note Organization:} To use document intelligence models (LayoutLMv3, MiniLMv6) to automatically extract, structure, and tag content from syllabi and uploaded notes.
...
The architecture of TrackEneer implements a modern microservices design pattern to ensure modularity, independent scalability, and fault isolation. The system comprises three primary layers: a presentation layer (frontend), a distributed service layer (backend microservices), and an intelligence layer (AI models and databases). Figure \ref{fig:architecture} illustrates the high-level system architecture and data flow.
...
\subsection{Backend}
The backend service layer implements a microservices architecture using \textbf{FastAPI} and \textbf{Flask} frameworks, providing modular, scalable services. The system comprises four independent backend services:
\begin{itemize}
    \item \textbf{Scheduler Service (Port 5000):} FastAPI-based service managing tasks, schedules, and deadlines with Neo4j integration
    \item \textbf{Placement Service (Port 5003):} FastAPI service for AI-powered company research using Gemini AI
    \item \textbf{Insights Service (Port 5004):} FastAPI service generating personalized academic guidance with Cohere AI
    \item \textbf{Study Service (Port 5001):} FastAPI service managing notes, subjects, and study materials
    \item \textbf{Notification Service (Port 3001):} Express/TypeScript service handling Web Push notifications via WebSocket
\end{itemize}

Data persistence is handled through a multi-database approach: \textbf{Neo4j} graph database for task relationships and scheduling, \textbf{MongoDB} for document storage, \textbf{ChromaDB} for vector embeddings, and JSON file storage for cached AI-generated content.

\subsection{AI Layer}
The AI layer represents the core intellectual capital of TrackEneer, implementing multiple specialized models distributed across microservices:
\begin{itemize}
...
The microservices architecture demonstrates excellent modularity and scalability. Each service operates independently on dedicated ports, allowing for parallel development and deployment. Table \ref{tab:service_ports} summarizes the service distribution and technologies employed.

\begin{table}[htbp]
\caption{TrackEneer Microservices Architecture}
...
\hline
Scheduler & 5000 & FastAPI, Neo4j, ChromaDB, spaCy \\
\hline
Study Module & 5001 & FastAPI, JSON Storage \\
\hline
Placement & 5003 & FastAPI, Gemini AI \\
\hline
Insights & 5004 & FastAPI, Cohere AI \\
\hline
Notifications & 3001 & Express, TypeScript, Web Push \\
\hline
\end{tabular}
\end{center}
\end{table}
...
While the current implementation focuses on core functionality, production deployment would benefit from additional security measures including HTTPS enforcement, rate limiting on API endpoints, database access control lists, encrypted storage of sensitive user data, and comprehensive audit logging. The microservices architecture facilitates adding these security layers without disrupting existing functionality.
...
\begin{itemize}
    ...
    \item \textbf{Time Optimization:} The intelligent caching system (24-hour validity for AI-generated content) reduces redundant API calls by 80-90\%, demonstrating efficient resource utilization. The microservices architecture enables selective module activation, reducing server resource consumption when certain features are not in use.
    ...
    \item \textbf{Scalability for Institutional Deployment:} The microservices architecture and caching mechanisms make TrackEneer suitable for institutional deployment serving thousands of students. The distributed service design enables horizontal scaling, while intelligent caching dramatically reduces infrastructure costs at scale.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{CORS Configuration:} The microservices architecture required careful CORS configuration to allow cross-origin requests from the Next.js frontend (port 3000) to multiple backend services (ports 5000-5004, 3001). This was addressed through explicit CORS middleware configuration in each FastAPI/Express service.
\end{itemize}

\subsection{Lessons Learned}
\begin{itemize}
    \item \textbf{Microservices Modularity:} The decision to separate concerns into independent services proved invaluable for parallel development, independent scaling, and fault isolation. Issues in one module do not affect others.
...
\section{Future Work}
While TrackEneer has successfully implemented core functionality across all modules, several enhancements are planned for future releases:
\begin{itemize}
    \item \textbf{Document Intelligence Integration:} Integration of LayoutLMv3 or similar models for automatic syllabus extraction and structuring from PDF uploads in the study module
    \item \textbf{Advanced Analytics Dashboard:} Implementation of progress tracking, study pattern analysis, and performance metrics visualization
    \item \textbf{Collaborative Features:} Addition of group study tools, shared notes, and collaborative task management
    \item \textbf{Mobile Application:} Development of native mobile apps for iOS and Android with offline support
    \item \textbf{Enhanced Recommendation System:} Machine learning-based recommendations for study schedules based on historical performance and learning patterns
    \item \textbf{Interview Practice Module:} AI-powered mock interview system with speech recognition and feedback
    \item \textbf{Resume Builder and Analyzer:} Automated resume optimization using NLP to match job descriptions
    \item \textbf{Integration with Learning Management Systems:} API integrations with popular LMS platforms for automatic assignment import
    \item \textbf{Calendar Synchronization:} Two-way sync with Google Calendar, Outlook, and other calendar services
    \item \textbf{Gamification:} Achievement system, streaks, and leaderboards to increase student engagement
\end{itemize}

\section{Conclusion}
This paper has detailed the design, architecture, implementation, and evaluation of TrackEneer, an AI-powered academic companion that successfully bridges the gap between academic organization and placement readiness for engineering students. The system implements a comprehensive microservices architecture featuring four specialized backend services (scheduler, study, placement, insights) integrated with a modern Next.js frontend and real-time notification service.
